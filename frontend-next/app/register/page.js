...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setSuccess(
        'Enterprise account created successfully! Redirecting to dashboard...'
      )
  ...
  const inputClass =
    'w-full bg-white/[0.03] border border-white/10 rounded-lg px-4 py-3 text-white placeholder-white/30 text-sm outline-none focus:border-[#4f9cf9] focus:ring-2 focus:ring-[#4f9cf9]/20 transition-all'
  ...
  return (
    ...
    <p className="text-white/50 text-sm">Join Infosys RiskIntelligence for automated property research</p>
        ...
        <div className="bg-[rgba(11,19,43,0.65)] backdrop-blur-xl border border-white/10 rounded-2xl p-8 shadow-2xl">
          ...
          {success && <div className="mb-4 flex items-center gap-2 bg-green-500/10 border border-green-500/30 text-green-300 rounded-lg px-4 py-3 text-sm"><span>✓</span>{success}</div>}
          ...
          <form onSubmit={handleSubmit} noValidate className="flex flex-col gap-4">
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <input id="reg-fullName" name="fullName" type="text" placeholder="e.g. Abhimanyu Kumar Ray" value={form.fullName} onChange={handleChange} required autoComplete="name" className={inputClass} />
            </div>

            <div className="flex flex-col gap-1.5">
              <label htmlFor="reg-email" className="text-xs font-bold tracking-widest uppercase text-white/50">Work Email Address</label>
              <input id="reg-email" name="email" type="email" placeholder="name@company.com" value={form.email} onChange={handleChange} required autoComplete="email" className={inputClass} />
            </div>

            <div className="flex flex-col gap-1.5">
              <label htmlFor="reg-password" className="text-xs font-bold tracking-widest uppercase text-white/50">Password</label>
              <input id="reg-password" name="password" type="password" placeholder="Min. 8 characters" value={form.password} onChange={handleChange} required minLength={8} autoComplete="new-password" className={inputClass} />
            </div>
            ...
            <button type="submit" disabled={loading} className="w-full mt-2 py-3 px-6 bg-[#4f9cf9] hover:bg-[#2d7ef4] text-white font-semibold rounded-lg transition-all hover:-translate-y-0.5 hover:shadow-[0_8px_30px_rgba(79,156,249,0.35)] disabled:opacity-60 disabled:cursor-not-allowed">{loading ? 'Creating Account...' : 'Register Enterprise Account'}</button>
          </form>

          <p className="mt-6 text-center text-sm text-white/40">Already have an account? <Link href="/login" className="text-[#4f9cf9] hover:underline">Sign in</Link></p>
        </div>
      </div>
    </main>
  )
